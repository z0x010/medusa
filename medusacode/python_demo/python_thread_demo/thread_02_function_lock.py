...
def print_datetime(thread_name, delay, count, lock):
    lock.acquire()
    c = 0
    while c < count:
        time.sleep(delay)
...
def main():
    ...
    try:
        thread_id_1 = thread.start_new_thread(print_datetime, ('Thread-1', 0.5, 3, thread_lock))
        thread_id_2 = thread.start_new_thread(print_datetime, ('Thread-2', 0.5, 5, thread_lock))
    except Exception, e:
        print e
        pass
